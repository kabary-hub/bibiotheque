...
until mysql -h "$DB_HOST" -u root -p"$DB_PASS" "$DB_NAME" \
        -e "SELECT 1 FROM books LIMIT 1;" >/dev/null 2>&1
do
    attempt=$((attempt + 1))
    if [ "$attempt" -ge "$MAX_ATTEMPTS" ]; then
        echo "[seed] ECHEC : les tables n'existent toujours pas apres $((MAX_ATTEMPTS * 3))s." >&2
